...
import { buildZaloArchiveSnapshot } from "./archive-snapshot";
import { buildZaloRecipeContractSnapshot } from "./contract-snapshot";

function buildIssue(code: string, message: string): RecipeContractParityIssue {
  return { code, message };
}
export async function buildZaloRecipeParityReport(): Promise<RecipeContractParityReport> {
  const contractSnapshot = buildZaloRecipeContractSnapshot();
  const archiveSnapshot = await buildZaloArchiveSnapshot();
  ...
  if (archiveSnapshot.packageInfo.config?.serviceURL !== "https://chat.zalo.me/") issues.push(buildIssue("service-url-mismatch", `Archive serviceURL "${String(archiveSnapshot.packageInfo.config?.serviceURL ?? "")}" does not match expected "https://chat.zalo.me/".`));
