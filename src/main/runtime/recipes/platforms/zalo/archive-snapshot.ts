...
const ZALO_ARCHIVE_SELECTOR_HINTS = [
  "#main-tab .nav__tabs__zalo",
  "#chatView",
  "#sidebarNav",
  '#messageViewContainer .text-message__container div[data-component="message-text-content"]',
  "#richInput",
  '.chat-box-input-button[data-translate-title="STR_SEND"]',
];
const ZALO_ARCHIVE_METHOD_HINTS = ["getChatInfo", "getMsgTxt", "getIsMe", "getInputMsg", "getUnreadCount"];

function getProjectRootDir(): string {
  return path.resolve(__dirname, "../../../../../..");
}
function getArchivePath(): string {
  return path.join(getProjectRootDir(), "recipes", "archives", "zalo.tar.gz");
}
...
export async function buildZaloArchiveSnapshot(): Promise<RecipeArchiveSnapshot> {
  const archivePath = getArchivePath();
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "yunyi-zalo-archive-"));
  try {
    ...
    return {
      ...
      archivePath,
      packageInfo,
      selectorHints: collectHintMatches(webviewSource, ZALO_ARCHIVE_SELECTOR_HINTS),
      methodHints: collectHintMatches(webviewSource, ZALO_ARCHIVE_METHOD_HINTS),
    };
  } finally {
