...
const GOOGLECHAT_ARCHIVE_SELECTOR_HINTS = [
  ".BCL84c.aM97s.ZCbGl.zmXnlc.Ahbcuc",
  ".Zc1Emd.QIJiHb",
  ".hj99tb.KRoqRc",
];

const GOOGLECHAT_ARCHIVE_METHOD_HINTS = [
  "getChatInfo",
  "getMsgTxt",
  "getIsMe",
  "getInputMsg",
...
function getArchivePath(): string {
  return path.join(getProjectRootDir(), "recipes", "archives", "googlechat.tar.gz");
}
...
export async function buildGoogleChatArchiveSnapshot(): Promise<RecipeArchiveSnapshot> {
  const archivePath = getArchivePath();
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "yunyi-googlechat-archive-"));
  try {
    ...
    return {
      ...
      archivePath,
      packageInfo,
      selectorHints: collectHintMatches(webviewSource, GOOGLECHAT_ARCHIVE_SELECTOR_HINTS),
      methodHints: collectHintMatches(webviewSource, GOOGLECHAT_ARCHIVE_METHOD_HINTS),
    };
  } finally {
