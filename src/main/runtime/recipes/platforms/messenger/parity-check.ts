...
import { buildMessengerArchiveSnapshot } from "./archive-snapshot";
import { buildMessengerRecipeContractSnapshot } from "./contract-snapshot";

function buildIssue(code: string, message: string): RecipeContractParityIssue {
  return { code, message };
}

export async function buildMessengerRecipeParityReport(): Promise<RecipeContractParityReport> {
  const contractSnapshot = buildMessengerRecipeContractSnapshot();
  const archiveSnapshot = await buildMessengerArchiveSnapshot();
  ...
  if (archiveSnapshot.packageInfo.config?.serviceURL !== "https://www.messenger.com/") {
    issues.push(buildIssue("service-url-mismatch", `Archive serviceURL "${String(archiveSnapshot.packageInfo.config?.serviceURL ?? "")}" does not match expected "https://www.messenger.com/".`));
  }
