...
import { buildFacebookArchiveSnapshot } from "./archive-snapshot";
import { buildFacebookRecipeContractSnapshot } from "./contract-snapshot";

function buildIssue(code: string, message: string): RecipeContractParityIssue {
  return { code, message };
}

export async function buildFacebookRecipeParityReport(): Promise<RecipeContractParityReport> {
  const contractSnapshot = buildFacebookRecipeContractSnapshot();
  const archiveSnapshot = await buildFacebookArchiveSnapshot();
  ...
  if (archiveSnapshot.packageInfo.config?.serviceURL !== "https://www.facebook.com/login") {
    issues.push(buildIssue("service-url-mismatch", `Archive serviceURL "${String(archiveSnapshot.packageInfo.config?.serviceURL ?? "")}" does not match expected "https://www.facebook.com/login".`));
  }
