...
import { buildTwitterArchiveSnapshot } from "./archive-snapshot";
import { buildTwitterRecipeContractSnapshot } from "./contract-snapshot";

function buildIssue(code: string, message: string): RecipeContractParityIssue {
  return { code, message };
}

export async function buildTwitterRecipeParityReport(): Promise<RecipeContractParityReport> {
  const contractSnapshot = buildTwitterRecipeContractSnapshot();
  const archiveSnapshot = await buildTwitterArchiveSnapshot();
  ...
  if (archiveSnapshot.packageInfo.config?.serviceURL !== "https://twitter.com") {
    issues.push(
      buildIssue(
        "service-url-mismatch",
        `Archive serviceURL "${String(archiveSnapshot.packageInfo.config?.serviceURL ?? "")}" does not match expected "https://twitter.com".`,
      ),
    );
  }
