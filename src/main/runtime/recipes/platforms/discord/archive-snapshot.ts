...
const DISCORD_ARCHIVE_SELECTOR_HINTS = [
  '[class*="nameTag_"] [class*="panelTitleContainer_"]',
  '[class*="chatContent_"]',
  '[class*="sidebar_"] [class*="sidebarList_"] ul[role="list"]',
  "nav.wrapper_ef3116.guilds_c48ade,.peopleList__5ec2f",
  '[class*="messagesWrapper_"] [class*="scrollerInner_"]',
  ".editor__1b31f.slateTextArea_ec4baf",
];

const DISCORD_ARCHIVE_METHOD_HINTS = [
  "getChatInfo",
  "getMsgTxt",
  "getIsMe",
  "getInputMsg",
...
function getArchivePath(): string {
  return path.join(getProjectRootDir(), "recipes", "archives", "discord.tar.gz");
}
...
export async function buildDiscordArchiveSnapshot(): Promise<RecipeArchiveSnapshot> {
  const archivePath = getArchivePath();
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "yunyi-discord-archive-"));
  try {
    ...
    return {
      ...
      archivePath,
      packageInfo,
      selectorHints: collectHintMatches(webviewSource, DISCORD_ARCHIVE_SELECTOR_HINTS),
      methodHints: collectHintMatches(webviewSource, DISCORD_ARCHIVE_METHOD_HINTS),
    };
  } finally {
