...
import { buildDiscordArchiveSnapshot } from "./archive-snapshot";
import { buildDiscordRecipeContractSnapshot } from "./contract-snapshot";

function buildIssue(code: string, message: string): RecipeContractParityIssue {
  return { code, message };
}

export async function buildDiscordRecipeParityReport(): Promise<RecipeContractParityReport> {
  const contractSnapshot = buildDiscordRecipeContractSnapshot();
  const archiveSnapshot = await buildDiscordArchiveSnapshot();
  ...
  if (archiveSnapshot.packageInfo.config?.serviceURL !== "https://discord.com/app") {
    issues.push(buildIssue("service-url-mismatch", `Archive serviceURL "${String(archiveSnapshot.packageInfo.config?.serviceURL ?? "")}" does not match expected "https://discord.com/app".`));
  }
