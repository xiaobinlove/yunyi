...
import { buildInstagramArchiveSnapshot } from "./archive-snapshot";
import { buildInstagramRecipeContractSnapshot } from "./contract-snapshot";
...
export async function buildInstagramRecipeParityReport(): Promise<RecipeContractParityReport> {
  const contractSnapshot = buildInstagramRecipeContractSnapshot();
  const archiveSnapshot = await buildInstagramArchiveSnapshot();
  ...
  const expectedServiceUrl = "https://instagram.com/";
