...
const TINDER_ARCHIVE_SELECTOR_HINTS = [
  "aside",
  ".messageList",
  ".chat",
  ".msgHelper .msg .text",
...
const TINDER_ARCHIVE_METHOD_HINTS = [
  "getChatInfo",
  "getIsMe",
  "getUnreadCount",
] as const;
...
function getArchivePath(): string {
  return path.join(getProjectRootDir(), "recipes", "archives", "tinder.tar.gz");
}
...
export async function buildTinderArchiveSnapshot(): Promise<RecipeArchiveSnapshot> {
  const archivePath = getArchivePath();
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "yunyi-tinder-archive-"));
  try {
    ...
    return {
      ...
      archivePath,
      packageInfo,
      selectorHints: collectHintMatches(webviewSource, TINDER_ARCHIVE_SELECTOR_HINTS),
      methodHints: collectHintMatches(webviewSource, TINDER_ARCHIVE_METHOD_HINTS),
    };
  } finally {
