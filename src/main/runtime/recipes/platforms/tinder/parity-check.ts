...
import { buildTinderArchiveSnapshot } from "./archive-snapshot";
import { buildTinderRecipeContractSnapshot } from "./contract-snapshot";

function buildIssue(code: string, message: string): RecipeContractParityIssue {
  return { code, message };
}

export async function buildTinderRecipeParityReport(): Promise<RecipeContractParityReport> {
  const contractSnapshot = buildTinderRecipeContractSnapshot();
  const archiveSnapshot = await buildTinderArchiveSnapshot();
  ...
  if (
    archiveSnapshot.packageInfo.config?.serviceURL !==
    "chrome-extension://cfohepagpmnodfdmjliccbbigdkfcgia/options.html"
  ) {
    issues.push(
      buildIssue(
        "service-url-mismatch",
        `Archive serviceURL "${String(archiveSnapshot.packageInfo.config?.serviceURL ?? "")}" does not match expected "chrome-extension://cfohepagpmnodfdmjliccbbigdkfcgia/options.html".`,
      ),
    );
  }
