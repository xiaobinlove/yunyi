...
import { buildSnapchatArchiveSnapshot } from "./archive-snapshot";
import { buildSnapchatRecipeContractSnapshot } from "./contract-snapshot";

function buildIssue(code: string, message: string): RecipeContractParityIssue {
  return { code, message };
}

export async function buildSnapchatRecipeParityReport(): Promise<RecipeContractParityReport> {
  const contractSnapshot = buildSnapchatRecipeContractSnapshot();
  const archiveSnapshot = await buildSnapchatArchiveSnapshot();
  ...
  if (archiveSnapshot.packageInfo.config?.serviceURL !== "https://web.snapchat.com/") {
    issues.push(
      buildIssue(
        "service-url-mismatch",
        `Archive serviceURL "${String(archiveSnapshot.packageInfo.config?.serviceURL ?? "")}" does not match expected "https://web.snapchat.com/".`,
      ),
    );
  }
