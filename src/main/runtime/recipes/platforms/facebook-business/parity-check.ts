...
import { buildFacebookBusinessArchiveSnapshot } from "./archive-snapshot";
import { buildFacebookBusinessRecipeContractSnapshot } from "./contract-snapshot";

function buildIssue(code: string, message: string): RecipeContractParityIssue {
  return { code, message };
}

export async function buildFacebookBusinessRecipeParityReport(): Promise<RecipeContractParityReport> {
  const contractSnapshot = buildFacebookBusinessRecipeContractSnapshot();
  const archiveSnapshot = await buildFacebookBusinessArchiveSnapshot();
  ...
  if (archiveSnapshot.packageInfo.config?.serviceURL !== "https://business.facebook.com/latest") {
    issues.push(
      buildIssue(
        "service-url-mismatch",
        `Archive serviceURL "${String(archiveSnapshot.packageInfo.config?.serviceURL ?? "")}" does not match expected "https://business.facebook.com/latest".`,
      ),
    );
  }
